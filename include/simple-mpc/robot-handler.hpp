...
public:
   ...
   */
  RobotModelHandler(const Model &model,
   ...
   * reference frame
   */
  FrameIndex addFoot(const std::string &foot_name,
  ...
  size_t getFootNb(const std::string &foot_name) const {
    return std::find(feet_names_.begin(), feet_names_.end(), foot_name) -
           feet_names_.begin();
  }

  const std::vector<FrameIndex> &getFeetIds() const { return feet_ids_; }

  const std::string &getFootName(size_t i) const { return feet_names_.at(i); }
  ...
  const Model &getCompleteModel() const { return model_full_; }
};
