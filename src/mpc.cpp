...
void MPC::initialize(const MPCSettings &settings, const RobotHandler &handler,
  ...
  for (unsigned long i = 0; i < handler_.get_ee_ids().size(); i++) {
    ref_frame_poses_[i].assign(problem_->numSteps() + 1,
  ...
  SolverProxDDPTpl<double> solver(settings_.TOL, settings_.mu_init, 0.,
...
static_cast<double>(settings_.TdoubleSupport) + min_min_force * (i + 1) /
static_cast<double>(settings_.TdoubleSupport); wrench_reference_right[2] =
ref_force; wrench_reference_left[2] = settings_.support_force - ref_force;
  ...
  for (int j = 1; j < settings_.totalSteps; j++) {
    for (int i = 0; i < settings_.TdoubleSupport; i++) {
      ref_force = max_min_force * (settings_.TdoubleSupport - i - 1) /
static_cast<double>(settings_.TdoubleSupport) + min_min_force * (i + 1) /
static_cast<double>(settings_.TdoubleSupport); if (j % 2 == 0) {
...
wrench_reference_left); fullHorizon_.setWrenchReference(i + j * settings_.Tstep,
"wrench_RF", wrench_reference_right);
  ...
  for (int i = 0; i < settings_.TdoubleSupport; i++) {
    ref_force = max_min_force * (settings_.TdoubleSupport - i - 1) /
static_cast<double>(settings_.TdoubleSupport) + mean_force * (i + 1) /
static_cast<double>(settings_.TdoubleSupport); if (settings_.totalSteps % 2 ==
0) { wrench_reference_right[2] = ref_force; wrench_reference_left[2] =
settings_.support_force - ref_force; } else { wrench_reference_right[2] =
ref_force; wrench_reference_left[2] = settings_.support_force - ref_force;
    }
    fullHorizon_.setWrenchReference(i + settings_.totalSteps * settings_.Tstep,
"wrench_LF", wrench_reference_left); fullHorizon_.setWrenchReference(i +
settings_.totalSteps * settings_.Tstep, "wrench_RF", wrench_reference_right);
  }
} */
...
settings_.T); land_RF_.push_back((j + 1) * settings_.Tstep + settings_.T); }
else { takeoff_LF_.push_back(j * settings_.Tstep + settings_.TdoubleSupport +
settings_.T); land_LF_.push_back((j + 1) * settings_.Tstep + settings_.T);
    }
...
designer_.get_RF_name()}; fullHorizon_ = HorizonManager(names, x0_,
cyclicModels, cyclicModels.back()); setForceAlongHorizon();
}
...
void MPC::iterate(const Eigen::VectorXd &q_current, const Eigen::VectorXd
&v_current, bool is_feasible) { x0_ = designer_.shapeState(q_current,
v_current);
...
void MPC::iterate(int iteration, const Eigen::VectorXd &q_current, const
Eigen::VectorXd &v_current, bool is_feasible) { if (timeToSolveDDP(iteration)) {
...
getPoseRef_RF(horizon_.size()));
  ...
  if (horizon_.contacts(horizon_.size() -
1)->getContactStatus(designer_.get_LF_name()) and
      horizon_.contacts(horizon_.size() -
1)->getContactStatus(designer_.get_RF_name())) { ref_dcm_ =
(getPoseRef_LF(horizon_.size()).translation() +
getPoseRef_RF(horizon_.size()).translation()) / 2; } else if
(horizon_.contacts(horizon_.size() -
1)->getContactStatus(designer_.get_LF_name())) { ref_dcm_ =
getPoseRef_LF(horizon_.size()).translation(); } else if
(horizon_.contacts(horizon_.size() -
1)->getContactStatus(designer_.get_RF_name())) { ref_dcm_ =
...
getPoseRef_LF(horizon_.size() - 1)); horizon_.setPoseReference(horizon_.size() -
1, "placement_RF", getPoseRef_RF(horizon_.size() - 1));
  horizon_.setTerminalPoseReference("placement_LF",
getPoseRef_LF(horizon_.size()));
  horizon_.setTerminalPoseReference("placement_RF",
getPoseRef_RF(horizon_.size())); ref_LF_poses_.erase(ref_LF_poses_.begin());
...
fullHorizon_.ada(horizon_iteration_)); horizon_iteration_++; } else
    horizon_.recede();
}

void MPC::goToNextDoubleSupport() {
  while (horizon_.supportSize(0) != 2 and horizon_iteration_ <
fullHorizon_.size()) { horizon_.recede(fullHorizon_.ama(horizon_iteration_),
fullHorizon_.ada(horizon_iteration_)); horizon_iteration_++;
    updateSupportTiming();
  }
  horizon_.recede(fullHorizon_.ama(horizon_iteration_),
fullHorizon_.ada(horizon_iteration_)); horizon_iteration_++;
