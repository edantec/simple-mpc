...
BOOST_AUTO_TEST_CASE(model_handler) {
  ...
  const std::vector<std::string> locked_joints{
      "FR_HFE", "FL_HFE"}; // Lock two random joint for testing
  const std::vector<std::string> feet_names{"FR_FOOT", "FL_FOOT", "HR_FOOT",
                                            "HL_FOOT"};
  ...
  // Base frame
  {
    BOOST_CHECK_EQUAL(model.frames.at(model_handler.getBaseFrameId()).name,
                      base_frame);
  }
  ...
  // Feet tests
  {
    for (size_t i = 0; i < feet_names.size(); i++) {
      ...
      BOOST_CHECK_EQUAL(foot_name,
                        model.frames.at(model_handler.getFeetIds().at(i)).name);
      BOOST_CHECK_EQUAL(
          foot_name, model.frames.at(model_handler.getFootId(foot_name)).name);

      const FrameIndex ref_frame = model_handler.getRefFootId(foot_name);
      const FrameIndex ref_frame_parent =
          model_handler.getModel().frames.at(ref_frame).parentFrame;
      BOOST_CHECK_EQUAL(
          model_handler.getModel().frames.at(ref_frame_parent).name,
          base_frame);
      BOOST_CHECK(
          model_handler.getModel().frames.at(ref_frame).placement.isApprox(
              feet_refs.at(i)));
    }
  }
  ...
  // State
  {
    ...
    for (int n = 0; n < 2;
         n++) // First time with random data, second with reference state
    {
      ...
      for (int i = 1; i < model_handler.getModel().njoints; i++) {
        ...
        const JointModel &joint_full =
            model.joints[model.getJointId(joint_name)];
        const JointModel &joint_red = model_handler.getModel().joints[i];

        x.segment(joint_red.idx_q(), joint_red.nq()) =
            q.segment(joint_full.idx_q(), joint_full.nq());
        x.segment(nq_red + joint_red.idx_v(), joint_red.nv()) =
            v.segment(joint_full.idx_v(), joint_full.nv());
      }
  ...
  // Difference
  {
    Eigen::Vector<double, 17> q1 =
        pinocchio::randomConfiguration(model_handler.getModel());
    q1.head<3>() = Eigen::Vector3d::Random();
    Eigen::Vector<double, 17> q2 =
        pinocchio::randomConfiguration(model_handler.getModel());
...
BOOST_AUTO_TEST_CASE(data_handler) {
  ...
  Eigen::Vector<double, 19> q =
      pinocchio::randomConfiguration(model_handler.getCompleteModel());
  q.head<3>() = Eigen::Vector3d::Random();
  ...
  // Foot pose
  {
    ...
    for (std::string foot_name : feet_names) {
      ...
      BOOST_CHECK(
          data.oMf[foot_id].isApprox(data_handler.getFootPose(foot_name)));
      BOOST_CHECK(data.oMf[ref_foot_id].isApprox(
          data_handler.getRefFootPose(foot_name)));
    }
  }
  ...
  // Centroidal state
  {
    ...
    const Eigen::Vector<double, 9> x_centroidal =
        data_handler.getCentroidalState();
