...
# Create a cache variable with not default for a path, with a fallback when
# unset; used for entries slaved to other entries such as DATAROOTDIR.
...
_gnuinstalldirs_cache_path(CMAKE_INSTALL_BINDIR "bin" "User executables (bin)")
...
# We check if the variable was manually set and not cached, in order to allow
# projects to set the values as normal variables before including GNUInstallDirs
# to avoid having the entries cached or user-editable. It replaces the "if(NOT
# DEFINED CMAKE_INSTALL_XXX)" checks in all the other cases. If
# CMAKE_INSTALL_LIBDIR is defined, if _libdir_set is false, then the variable is
# a normal one, otherwise it is a cache one.
get_property(
  _libdir_set
...
if(NOT DEFINED CMAKE_INSTALL_LIBDIR
  ...
  # If CMAKE_INSTALL_LIBDIR is not defined, it is always executed. Otherwise: *
  # if _libdir_set is false it is not executed (meaning that it is not a cache
  # variable) * if _GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX is not defined it
  # is not executed * if _GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX and
  # CMAKE_INSTALL_PREFIX are the same string it is not executed.
  # _GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX is updated after the execution, of
  # this part of code, therefore at the next inclusion of the file,
  # CMAKE_INSTALL_LIBDIR is defined, and the 2 strings are equal, meaning that
  # the if is not executed the code the second time.

  set(_LIBDIR_DEFAULT "lib")
  # Override this default 'lib' with 'lib64' iff: - we are on Linux system but
  # NOT cross-compiling - we are NOT on debian - we are on a 64 bits system
  # reason is: amd64 ABI: https://github.com/hjl-tools/x86-psABI/wiki/X86-psABI
  # For Debian with multiarch, use 'lib/${CMAKE_LIBRARY_ARCHITECTURE}' if
  # CMAKE_LIBRARY_ARCHITECTURE is set (which contains e.g. "i386-linux-gnu" and
  ...
  if(DEFINED _GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX)
    ...
    # _GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX, not a cache entry for the value
    # that was last used as the default. This value is used to figure out
    # whether the user changed the CMAKE_INSTALL_LIBDIR value manually, or if
    # the value was the default one. When CMAKE_INSTALL_PREFIX changes, the
    # value is updated to the new default, unless the user explicitly changed
    # it.
  endif()
  ...
  elseif(DEFINED __LAST_LIBDIR_DEFAULT AND "${__LAST_LIBDIR_DEFAULT}" STREQUAL
                                           "${CMAKE_INSTALL_LIBDIR}")
    set_property(CACHE CMAKE_INSTALL_LIBDIR PROPERTY VALUE "${_LIBDIR_DEFAULT}")
  endif()
endif()
_gnuinstalldirs_cache_convert_to_path(CMAKE_INSTALL_LIBDIR
                                      "Object code libraries (lib)")

# Save for next run
set(_GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX
    "${CMAKE_INSTALL_PREFIX}"
    CACHE INTERNAL "CMAKE_INSTALL_PREFIX during last run")
unset(_libdir_set)
...
# -----------------------------------------------------------------------------
# Values whose defaults are relative to DATAROOTDIR.  Store empty values in the
# cache and store the defaults in local variables if the cache values are not
# set explicitly.  This auto-updates the defaults as DATAROOTDIR changes.

_gnuinstalldirs_cache_path_fallback(
  CMAKE_INSTALL_DATADIR "${CMAKE_INSTALL_DATAROOTDIR}"
  "Read-only architecture-independent data (DATAROOTDIR)")
...
if(CMAKE_SYSTEM_NAME MATCHES "^(([^kF].*)?BSD|DragonFly)$")
  ...
  _gnuinstalldirs_cache_path_fallback(
    CMAKE_INSTALL_INFODIR "${CMAKE_INSTALL_DATAROOTDIR}/info"
    "Info documentation (DATAROOTDIR/info)")
endif()
...
_gnuinstalldirs_cache_path_fallback(
  CMAKE_INSTALL_RUNSTATEDIR "${CMAKE_INSTALL_LOCALSTATEDIR}/run"
  "Run-time variable data (LOCALSTATEDIR/run)")
...
macro(GNUInstallDirs_get_absolute_install_dir absvar var)
  if(NOT IS_ABSOLUTE "${${var}}")
    # Handle special cases: - CMAKE_INSTALL_PREFIX == / - CMAKE_INSTALL_PREFIX
    # == /usr - CMAKE_INSTALL_PREFIX == /opt/...
    if("${CMAKE_INSTALL_PREFIX}" STREQUAL "/")
      if("${dir}" STREQUAL "SYSCONFDIR"
         OR "${dir}" STREQUAL "LOCALSTATEDIR"
