...
macro(_PYTHON_FIND_FRAMEWORKS)
  ...
  if(APPLE)
    set(_pff_frameworks
        ${CMAKE_FRAMEWORK_PATH} $ENV{CMAKE_FRAMEWORK_PATH} ~/Library/Frameworks
    ...
    foreach(_pff_framework IN LISTS _pff_frameworks)
      if(EXISTS ${_pff_framework}/Python.framework)
        list(APPEND ${_PYTHON_PREFIX}_FRAMEWORKS
...
function(_PYTHON_VALIDATE_INTERPRETER)
  ...
  if(expected_version AND NOT python_name STREQUAL
                          "python${expected_version}${CMAKE_EXECUTABLE_SUFFIX}")
  ...
  if(CMAKE_SIZEOF_VOID_P
     AND "Development" IN_LIST ${_PYTHON_PREFIX}_FIND_COMPONENTS
...
function(_PYTHON_FIND_RUNTIME_LIBRARY _PYTHON_LIB)
  ...
  if(CMAKE_SYSTEM_NAME STREQUAL "Windows"
     OR (CMAKE_SYSTEM_NAME MATCHES "MSYS|CYGWIN"
...
if(DEFINED ${_PYTHON_PREFIX}_FIND_REGISTRY)
  if(NOT ${_PYTHON_PREFIX}_FIND_REGISTRY MATCHES "^(FIRST|LAST|NEVER)$")
    message(
      AUTHOR_WARNING
        "Find${_PYTHON_PREFIX}: ${${_PYTHON_PREFIX}_FIND_REGISTRY}: invalid value for '${_PYTHON_PREFIX}_FIND_REGISTRY'. 'FIRST', 'LAST' or 'NEVER' expected."
    )
...
if("Interpreter" IN_LIST ${_PYTHON_PREFIX}_FIND_COMPONENTS)
    ...
    if(APPLE AND _${_PYTHON_PREFIX}_FIND_FRAMEWORK STREQUAL "FIRST")
      find_program(
        ...
        NAMES python${_${_PYTHON_PREFIX}_VERSION}
              python${_${_PYTHON_PREFIX}_REQUIRED_VERSION_MAJOR} NAMES_PER_DIR
    ...
    if(WIN32 AND _${_PYTHON_PREFIX}_FIND_REGISTRY STREQUAL "FIRST")
      find_program(
        ...
        NAMES python${_${_PYTHON_PREFIX}_VERSION}
              ...
              ${_${_PYTHON_PREFIX}_IRON_PYTHON_NAMES} NAMES_PER_DIR
  ...
  if(NOT ${_PYTHON_PREFIX}_EXECUTABLE)
    # No specific version found. Retry with generic names try using HINTS
    find_program(
      ${_PYTHON_PREFIX}_EXECUTABLE
      ...
      PATH_SUFFIXES bin ${_${_PYTHON_PREFIX}_IRON_PYTHON_PATH_SUFFIXES}
      NO_SYSTEM_ENVIRONMENT_PATH NO_CMAKE_SYSTEM_PATH)
    # try using standard paths. NAMES_PER_DIR is not defined on purpose to have
    # a chance to find expected version. For example, typical systems have
    # 'python' for version 2.* and 'python3' for version 3.*. So looking for
    # names per dir will find, potentially, systematically 'python' (i.e.
  ...
  if(${_PYTHON_PREFIX}_Interpreter_FOUND)
    ...
    execute_process(
      COMMAND "${${_PYTHON_PREFIX}_EXECUTABLE}" -V
      RESULT_VARIABLE _${_PYTHON_PREFIX}_RESULT
  ...
  execute_process(
    COMMAND
      "${${_PYTHON_PREFIX}_EXECUTABLE}" -c
      "import sys; from distutils import sysconfig;sys.stdout.write(';'.join([sysconfig.get_python_lib(plat_specific=False,standard_lib=True),sysconfig.get_python_lib(plat_specific=True,standard_lib=True),sysconfig.get_python_lib(plat_specific=False,standard_lib=False),sysconfig.get_python_lib(plat_specific=True,standard_lib=False)]))"
    RESULT_VARIABLE _${_PYTHON_PREFIX}_RESULT
...
if("Compiler" IN_LIST ${_PYTHON_PREFIX}_FIND_COMPONENTS)
  ...
  # IronPython specific artifacts If IronPython interpreter is found, use its
  # path
  ...
  if(${_PYTHON_PREFIX}_COMPILER)
    ...
    execute_process(
      COMMAND "${_${_PYTHON_PREFIX}_VERSION_DIR}/version"
      WORKING_DIRECTORY "${_${_PYTHON_PREFIX}_VERSION_DIR}"
      RESULT_VARIABLE _${_PYTHON_PREFIX}_RESULT
...
# third step, search for the development artifacts Development environment is
# not compatible with IronPython interpreter
if("Development" IN_LIST ${_PYTHON_PREFIX}_FIND_COMPONENTS
   AND NOT ${_PYTHON_PREFIX}_INTERPRETER_ID STREQUAL "IronPython")
  list(
    APPEND
    _${_PYTHON_PREFIX}_CACHED_VARS
  ...
  if(DEFINED ${_PYTHON_PREFIX}_USE_STATIC_LIBS AND NOT WIN32)
    set(_${_PYTHON_PREFIX}_CMAKE_FIND_LIBRARY_SUFFIXES
  ...
  # consistency between interpreter and development environment
  ...
  set(_${_PYTHON_PREFIX}_HINTS
      "${_${_PYTHON_PREFIX}_PREFIX}" "${${_PYTHON_PREFIX}_ROOT_DIR}" ENV
      ${_PYTHON_PREFIX}_ROOT_DIR)
  ...
  foreach(_${_PYTHON_PREFIX}_VERSION IN LISTS _${_PYTHON_PREFIX}_FIND_VERSIONS)
    ...
    execute_process(
      COMMAND "${_${_PYTHON_PREFIX}_CONFIG}" --prefix
      RESULT_VARIABLE _${_PYTHON_PREFIX}_RESULT
      OUTPUT_VARIABLE _${_PYTHON_PREFIX}_PREFIX
      ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
    if(_${_PYTHON_PREFIX}_RESULT)
    ...
    # retrieve library
    execute_process(
      COMMAND "${_${_PYTHON_PREFIX}_CONFIG}" --ldflags
      RESULT_VARIABLE _${_PYTHON_PREFIX}_RESULT
    ...
    execute_process(
      COMMAND "${_${_PYTHON_PREFIX}_CONFIG}" --includes
      RESULT_VARIABLE _${_PYTHON_PREFIX}_RESULT
      OUTPUT_VARIABLE _${_PYTHON_PREFIX}_FLAGS
      ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
    if(NOT _${_PYTHON_PREFIX}_RESULT)
  ...
  if(NOT (${_PYTHON_PREFIX}_LIBRARY_RELEASE OR ${_PYTHON_PREFIX}_LIBRARY_DEBUG)
     OR NOT ${_PYTHON_PREFIX}_INCLUDE_DIR)
    foreach(_${_PYTHON_PREFIX}_VERSION IN
      ...
      if(WIN32)
        ...
        if(${_PYTHON_PREFIX}_LIBRARY_RELEASE)
          ...
          get_filename_component(
            _${_PYTHON_PREFIX}_PATH "${${_PYTHON_PREFIX}_LIBRARY_RELEASE}"
            DIRECTORY)
          find_library(
            ${_PYTHON_PREFIX}_LIBRARY_DEBUG
            NAMES python${_${_PYTHON_PREFIX}_VERSION_NO_DOTS}_d NAMES_PER_DIR
      ...
      if(${_PYTHON_PREFIX}_LIBRARY_RELEASE OR ${_PYTHON_PREFIX}_LIBRARY_DEBUG)
        ...
        foreach(_${_PYTHON_PREFIX}_LIB IN
                ITEMS ${_PYTHON_PREFIX}_LIBRARY_RELEASE
                      ${_PYTHON_PREFIX}_LIBRARY_DEBUG)
          if(${_${_PYTHON_PREFIX}_LIB})
            ...
            elseif(DEFINED CMAKE_LIBRARY_ARCHITECTURE
                   AND ${_${_PYTHON_PREFIX}_LIB} MATCHES
                       "^(.+)/lib/${CMAKE_LIBRARY_ARCHITECTURE}")
              list(APPEND _${_PYTHON_PREFIX}_INCLUDE_HINTS "${CMAKE_MATCH_1}")
            else()
    ...
    find_path(${_PYTHON_PREFIX}_INCLUDE_DIR NAMES Python.h)
  endif()

  if(${_PYTHON_PREFIX}_INCLUDE_DIR)
    # retrieve version from header file
    file(STRINGS "${${_PYTHON_PREFIX}_INCLUDE_DIR}/patchlevel.h"
         _${_PYTHON_PREFIX}_VERSION
         REGEX "^#define[ \t]+PY_VERSION[ \t]+\"[^\"]+\"")
    string(REGEX
           REPLACE "^#define[ \t]+PY_VERSION[ \t]+\"([^\"]+)\".*" "\\1"
    ...
    if(NOT ${_PYTHON_PREFIX}_Interpreter_FOUND
       AND NOT ${_PYTHON_PREFIX}_Compiler_FOUND)
      # set public version information
      set(${_PYTHON_PREFIX}_VERSION ${_${_PYTHON_PREFIX}_VERSION})
      set(${_PYTHON_PREFIX}_VERSION_MAJOR ${_${_PYTHON_PREFIX}_VERSION_MAJOR})
      set(${_PYTHON_PREFIX}_VERSION_MINOR ${_${_PYTHON_PREFIX}_VERSION_MINOR})
  ...
  _python_set_library_dirs(
    ${_PYTHON_PREFIX}_LIBRARY_DIRS ${_PYTHON_PREFIX}_LIBRARY_RELEASE
    ${_PYTHON_PREFIX}_LIBRARY_DEBUG)
  if(UNIX)
    if(${_PYTHON_PREFIX}_LIBRARY_RELEASE MATCHES
       "${CMAKE_SHARED_LIBRARY_SUFFIX}$"
       OR ${_PYTHON_PREFIX}_LIBRARY_RELEASE MATCHES
          "${CMAKE_SHARED_LIBRARY_SUFFIX}$")
  ...
  mark_as_advanced(
    ${_PYTHON_PREFIX}_RUNTIME_LIBRARY_RELEASE
    ${_PYTHON_PREFIX}_RUNTIME_LIBRARY_DEBUG ${_PYTHON_PREFIX}_INCLUDE_DIR)
  ...
  if(DEFINED _${_PYTHON_PREFIX}_CMAKE_FIND_LIBRARY_SUFFIXES)
    set(CMAKE_FIND_LIBRARY_SUFFIXES
        ${_${_PYTHON_PREFIX}_CMAKE_FIND_LIBRARY_SUFFIXES})
  endif()
endif()
...
# final validation
if(${_PYTHON_PREFIX}_VERSION_MAJOR
   AND NOT ${_PYTHON_PREFIX}_VERSION_MAJOR VERSION_EQUAL
       _${_PYTHON_PREFIX}_REQUIRED_VERSION_MAJOR)
  _python_display_failure(
...
if(_${_PYTHON_PREFIX}_CMAKE_ROLE STREQUAL "PROJECT")
  ...
  if("Development" IN_LIST ${_PYTHON_PREFIX}_FIND_COMPONENTS
     ...
     AND NOT TARGET ${_PYTHON_PREFIX}::Python)
    ...
    if(${_PYTHON_PREFIX}_LIBRARY_RELEASE MATCHES
       ...
       OR ${_PYTHON_PREFIX}_RUNTIME_LIBRARY_DEBUG)
    ...
    if(_${_PYTHON_PREFIX}_CONFIG AND _${_PYTHON_PREFIX}_LIBRARY_TYPE STREQUAL
                                     "STATIC")
      ...
      execute_process(
        ...
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
      if(NOT _${_PYTHON_PREFIX}_RESULT)
        string(REGEX MATCHALL "-[Ll][^ ]+" _${_PYTHON_PREFIX}_LINK_LIBRARIES
                     "${_${_PYTHON_PREFIX}_FLAGS}")
    ...
    #
    # PYTHON_ADD_LIBRARY (<name> [STATIC|SHARED|MODULE] src1 src2 ... srcN) It
    # is used to build modules for python.
    #
  ...
  if("NumPy" IN_LIST ${_PYTHON_PREFIX}_FIND_COMPONENTS
     ...
     AND TARGET ${_PYTHON_PREFIX}::Python)
    ...
    target_link_libraries(${_PYTHON_PREFIX}::NumPy
                          INTERFACE ${_PYTHON_PREFIX}::Python)
  endif()
endif()
